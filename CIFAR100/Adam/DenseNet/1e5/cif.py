...
def lr_schedule(epoch):
    ...
    K_ = ((nb_classes - 1) * Kz) / (nb_classes * batch_size) + 1e-5 * max_wt
    S_ = (nb_classes - 1) ** 2 / (nb_classes * batch_size) ** 2 * S + 2e-5 * max_wt * ((nb_classes - 1) * Kz) / (nb_classes * batch_size)
...
optimizer = Adam(lr=1e-3, decay=1e-5) # Using Adam instead of SGD to speed up training
...
# Load model
save_dir = "."
model_name = 'epoch-{epoch:03d}-1e5.h5'
...
model.save('epoch-300-1e5-densenet-k12.h5')
history_300 = model.history.history
with open('history-1e5.pkl', 'wb') as f:
