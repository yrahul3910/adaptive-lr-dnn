...
def lr_schedule(epoch):
    ...
    K_ = ((nb_classes - 1) * Kz) / (nb_classes * batch_size) + 1e-3 * max_wt
    S_ = (nb_classes - 1) ** 2 / (nb_classes * batch_size) ** 2 * S + 2e-3 * max_wt * ((nb_classes - 1) * Kz) / (nb_classes * batch_size)
...
model.compile(loss='categorical_crossentropy',
              optimizer=Adam(1e-3, decay=1e-3),  # we will compute this later
              metrics=['accuracy'])
